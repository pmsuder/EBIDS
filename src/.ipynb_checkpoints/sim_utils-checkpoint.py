...
def generate_latex_table(filename, table):
    ...
    transposing the original table (rows become columns and vice versa),
    rounding all values to two decimals,
    and write it to the specified filename.

    Parameters:
    - filename: str, path to the output .txt file
    - table: dict of dicts, where table[column][row] = (x, y)
    """

    # Extract original column and row labels
    column_labels = list(table.keys())
    row_labels = list(next(iter(table.values())).keys())

    # Transpose: new rows are old columns, new columns are old rows
    new_row_labels = column_labels
    new_column_labels = row_labels

    # Define column format with vertical bars (extra for row labels)
    col_format = "|c|" + "|".join(["c"] * len(new_column_labels)) + "|"
    ...
    # Header row
    latex += "Index & " + " & ".join(str(label) for label in new_column_labels) + " \\\\\n"
    latex += "\\hline\n"

    # Fill rows
    for new_row in new_row_labels:
        row_entries = []
        for new_col in new_column_labels:
            x, y = table[new_row][new_col]
            x_rounded = round(x, 2)
            y_rounded = round(y, 2)
            row_entries.append(f"${x_rounded} \\pm {y_rounded}$")

        latex += f"{new_row} & " + " & ".join(row_entries) + " \\\\\n"
        latex += "\\hline\n"
